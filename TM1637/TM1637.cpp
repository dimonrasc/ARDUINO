...
void TM1637::sendArray(int8_t sendData[]) {
  start();          //start signal sent to TM1637 from MCU
  writeByte(ADDR_AUTO);//
  stop();           //
  start();          //
...
void TM1637::display(uint8_t bit0, uint8_t bit1, uint8_t bit2, uint8_t bit3) {
	uint8_t dispArray[] = {bit0, bit1, bit2, bit3};	
	display(dispArray);
}
...
void TM1637::brightness(uint8_t brightness, uint8_t SetData, uint8_t SetAddr)
{
  Cmd_SetData = SetData;
  Cmd_SetAddr = SetAddr;
...
// ************************** ВСЯКИЕ ФУНКЦИИ *****************************
void TM1637::displayClock(uint8_t hrs, uint8_t mins) {
	...
	disp_time[2] = mins / 10;
	disp_time[3] = mins % 10;
	TM1637::display(disp_time);
}
void TM1637::displayClockScroll(uint8_t hrs, uint8_t mins, int delayms) {
	if (hrs > 99 || mins > 99) return 0;
	uint8_t disp_time[4];	
	if ((hrs / 10) == 0) disp_time[0] = 10;
	else disp_time[0] = (hrs / 10);
...
void TM1637::displayClockTwist(uint8_t hrs, uint8_t mins, int delayms) {
	if (hrs > 99 || mins > 99) return 0;
	uint8_t disp_time[4];	
	if ((hrs / 10) == 0) disp_time[0] = 10;
	else disp_time[0] = (hrs / 10);
...
void TM1637::scroll(int8_t DispData[], int delayms) {
	byte DispDataByte[4];
	for (byte i = 0; i < 4; i++) {
...
void TM1637::scroll(uint8_t BitAddr, int8_t DispData, int delayms) {
	byte DispDataByte = TubeTab[DispData];
	scrollByte(BitAddr, DispDataByte, delayms);
}

void TM1637::scrollByte(int8_t DispData[], int delayms) {
	byte lastBytes[4];
	byte step;
	byte stepArray[4];
...
void TM1637::scrollByte(uint8_t BitAddr, int8_t DispData, int delayms) {
...
void TM1637::twist(uint8_t BitAddr, int8_t DispData, int delayms) {
	twistByte(BitAddr, TubeTab[DispData], delayms);
}

void TM1637::twist(int8_t DispData[], int delayms) {
	byte newData[4];
	for (byte i = 0; i < 4; i++) {
...
void TM1637::twistByte(int8_t DispData[], int delayms) {
	byte step;
	byte stepArray[4];
	boolean changeByte[4] = {0, 0, 0, 0};
...
void TM1637::twistByte(uint8_t BitAddr, int8_t DispData, int delayms) {
	byte oldByte = lastData[BitAddr];
	byte newByte = DispData;
	byte step = oldByte;
